...
class BinaryTree {
    ...
    getDepths(node,depth=0,depths = {}){
        if(!node) return;
        depths[node.value] = depth;
        this.getDepths(node.left,depth+1,depths);
        this.getDepths(node.right,depth+1,depths);
        return depths;
    }
}
...
console.log('depths of the given value is :',tree.getDepth(tree.root, 6));
        console.log('depth of all is :',tree.getDepths(tree.root));
